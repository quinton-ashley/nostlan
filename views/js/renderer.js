...
module.exports = async function(opt) {
	...
	async function reset() {
		...
		let files = klawSync(prefs.wii.libs[0], {
			nodir: true
		});
		...
		await fs.outputFile(`${__rootDir}/usr/${sys}Games.json`, JSON.stringify(games));
	...
	async function load() {
		...
		if (await fs.exists(prefsPath)) {
			games = JSON.parse(await fs.readFile(gamesPaths[sys]));
...
├── Cemu
└── Yuzu`);
				prefs.wii.libs.push(openLib(sys));
			}
			await reset(sys);
		}
	}
	...
	await load('wii');
	await viewer.load(games, prefs, 'wii');
	$('#cvs').remove();
